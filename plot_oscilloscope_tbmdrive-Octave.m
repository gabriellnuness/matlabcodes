...
files = dir('*.csv');
##files = struct2table(files); %octave doesn't have this function
##files = files(:,1);
...
for i = 1:length(files)
    ...
    figure('Name', [filename(1:end-4), '_wave_and_FFT'],...
        'units','centimeters','position',[0 0  40 25]);
        subplot(4,2,1)
...
for i = 1:numel(figHandles)
    ...
    print(figHandles(i), ['print_',num2str(figHandles(i)),'.svg'],'-dsvg')%ok
##    print(figHandles(i), ['print_',num2str(figHandles(i)),'.eps'],'-depsc')%ok
    print(figHandles(i), ['print_',num2str(figHandles(i)),'.png'])
